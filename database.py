...
async def _get_single_connection():
    """Получение единственного глобального соединения с базой данных с улучшенной обработкой блокировок"""
    ...
    async with _global_db_lock:
        ...
        if (_connection_cache is None or 
            current_time - _last_connection_time > 300):
...
async def _execute_with_retry(operation_func, *args, **kwargs):
    """Выполнение операции с повторными попытками и улучшенной обработкой блокировок"""
    global _connection_cache  # Объявляем global в начале функции
    last_exception = None
